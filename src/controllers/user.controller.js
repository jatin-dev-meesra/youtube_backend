...
const generateAccessAndRefershTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something went wrong while genrating referesh and access token!"
    );
  }
};
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImageLocalPath) {
    throw new ApiError(400, "Cover ImageLocal file is missing");
  }
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        //I made one chnage to remove deprication warrning here because it accepts only string value
